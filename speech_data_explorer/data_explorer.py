    ...
    with open(data_filename, 'r', encoding='utf8') as f:
        for line in tqdm.tqdm(f):
            ...
            if estimate_audio:
                ...
                audio_bytes = util.transcode_to_wav(target_file)
                tmp = io.BytesIO(audio_bytes)
                data, samplerate = sf.read(tmp)

                signal, sr = librosa.load(tmp, sr=None)
...
def plot_signal(idx, data):
    ...
    try:
        ...
        audio_bytes = util.transcode_to_wav(filename)
        tmp = io.BytesIO(audio_bytes)
        audio, fs = librosa.load(tmp, sr=None)
